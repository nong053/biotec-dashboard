...
conn=DriverManager.getConnection(connectionURL,User,Pass);
	if(!conn.isClosed()){
		st = conn.createStatement();
		Query="CALL sp_ic_score_by_division("+ParamYear+","+ParamMonth+",'BIOTEC')";
		rs = st.executeQuery(Query);
		Integer i=0;
...
conn=DriverManager.getConnection(connectionURL,User,Pass);
	if(!conn.isClosed()){
		st = conn.createStatement();
		Query="CALL sp_ic_score_by_job_family("+ParamYear+","+ParamMonth+",'BIOTEC','','')";
		rs = st.executeQuery(Query);
...
conn=DriverManager.getConnection(connectionURL,User,Pass);
	if(!conn.isClosed()){
		st = conn.createStatement();
		Query="CALL sp_ic_score_by_output_type("+ParamYear+","+ParamMonth+",'BIOTEC','','')";
		rs = st.executeQuery(Query);
		Integer i=0;
...
conn=DriverManager.getConnection(connectionURL,User,Pass);
	if(!conn.isClosed()){
		st = conn.createStatement();
		Query="CALL sp_count_emp_all_vs_jf2000("+ParamYear+","+ParamMonth+",'BIOTEC','','')";
		rs = st.executeQuery(Query);
		Integer i=0;
...
conn=DriverManager.getConnection(connectionURL,User,Pass);
	if(!conn.isClosed()){
		st = conn.createStatement();
		Query="CALL sp_count_emp_by_job_grade("+ParamYear+","+ParamMonth+",'BIOTEC','','')";
		rs = st.executeQuery(Query);
		Integer i=0;
